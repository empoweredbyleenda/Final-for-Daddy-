...
const Results = () => {
  ...
  return (
    <section id="results" className="py-24 bg-stone-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-light text-white mb-4">
            ...
            <span className="block text-amber-400 font-semibold">Before & After</span>
          </h2>
          ...
          {beforeAfterImages.map((item, index) => (
            <div
              ...
              className={`relative aspect-[3/4] rounded-2xl overflow-hidden group cursor-pointer transition-all duration-300 ${
                index === currentSlide ? 'ring-2 ring-amber-400' : ''
              }`}
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-amber-500/20 flex items-center justify-center">
                      <span className="text-amber-400 text-2xl font-light">{index + 1}</span>
                    </div>
                    <p className="text-white font-medium">{item.title}</p>
                    <p className="text-stone-400 text-sm">{item.area}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between">
                ...
                <span className="bg-amber-500/80 backdrop-blur-sm text-white text-xs px-3 py-1 rounded-full">
                  After
                </span>
              </div>
            </div>
        ...
        <div className="flex justify-center gap-4">
          <Button
            ...
            size="icon"
            onClick={prevSlide}
            className="rounded-full border-stone-700 text-stone-400 hover:bg-stone-800 hover:text-white"
          >
            <ChevronLeft size={20} />
          </Button>
